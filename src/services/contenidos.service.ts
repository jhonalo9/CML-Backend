  ...
  try {
    ...
    const contenido = await prisma.contenido.create({
      data: {
        ...
        urlArchivo: resultadoUpload.webViewLink,
        urlEmbed: urlEmbed,
        driveFileId: resultadoUpload.id,
...
export const obtenerContenido = async (req: Request, res: Response) => {
  try {
    ...
    // Agregar información adicional de Google Drive
    let respuesta: any = { ...contenido };
    ...
    if (contenido.driveFileId) {
      try {
        ...
        respuesta.miniatura = driveInfo.thumbnailLink;
        respuesta.fechaModificacion = driveInfo.modifiedTime;
      } catch (error) {
        console.error('Error al obtener info de Drive:', error);
        // Si falla, al menos incluir enlaces básicos
        respuesta.enlaces = {
          ver: contenido.urlArchivo,
          embed: contenido.urlEmbed || null,
          descarga: contenido.driveFileId ? driveStorage.getDirectDownloadLink(contenido.driveFileId) : null
        };
      }
    }
  ...
  try {
    ...
    if (req.file && contenido.driveFileId) {
      try {
        ...
        return sendError(res, 'Error al actualizar el archivo en Google Drive', 500);
      }
    }
